...
def load_vla(
    ...
    # Otherwise =>> try looking for a match on `model_id_or_path` on the HF Hub (`VLA_HF_HUB_REPO`)
        ...
        overwatch.info("Checking HF for `{(hf_path := openvla/openvla-7b}`")
        ...
        with overwatch.local_zero_first():
            ...
            config_json = hf_hub_download(
                repo_id="openvla/openvla-7b", filename="config.json", cache_dir=cache_dir
            )
            dataset_statistics_json = hf_hub_download(
                repo_id="openvla/openvla-7b", filename="dataset_statistics.json", cache_dir=cache_dir
            )
            checkpoint_pt = hf_hub_download(
                repo_id="openvla/openvla-7b", filename="checkpoints/", cache_dir=cache_dir
            )
